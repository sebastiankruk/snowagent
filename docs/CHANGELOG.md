...
- **Signal protection** for `query_history` plugin: configurable `max_entries` top-N limiting, include/exclude filters for warehouses/databases/users, and `max_lookback_minutes` watermark window. Prevents overload on high-volume Snowflake accounts. Overload protection logs and bizevents are now emitted under `dsoa.run.context == "self_monitoring"` (previously `"query_history"`) so normal query telemetry is cleanly separable. The `max_entries_applied` flag is now only set when rows are actually dropped.
...
- **Warehouse change attribution** for `query_history` plugin (experimental): `track_ddl_changes` config key surfaces DDL change details (`snowflake.object.{type,name,ddl.operation,ddl.properties}`) on query spans using `ACCESS_HISTORY.OBJECT_MODIFIED_BY_DDL` (covers database-object DDL: tables, views, procedures). Ships with the new *Warehouse Change Detection* dashboard and `warehouse-sensitive-change-alert` workflow — these detect warehouse DDL via `db.operation.name` + `db.query.text` keyword matching (since Snowflake does not populate `ACCESS_HISTORY.OBJECT_MODIFIED_BY_DDL` for warehouse operations).
...
- **Per-database event table support** for `event_log` plugin (experimental, `discover_db_event_tables: false` opt-in): discovers DB-level `EVENT_TABLE` parameter overrides to prevent silent telemetry blackout on multi-tenant accounts.
...
- **Budgets & FinOps dashboard**: improved account filtering to include all plugins (not just budgets) and enhanced warehouse data collection from both events and logs.
